<x-app-layout>
    ...
    <div class="shadow rounded-sm w-full">
        <div class="flex justify-between items-center">
            <p class="font-bold py-2 m-4 text-secondaire text-xl">Utilisateurs</p>
        ...
        <table class="w-full bg-white" >
            <thead class="">
                <tr class="bg-first-orange text-white rounded">
                    ...
                    <th class="px-4 py-2 text-left">Tel</th>
            ...
            <tbody class="bg-white">
                ...
                @endphp
                    <tr class="border-b border-gray-200">
                        <td class=" flex px-4 py-3 border-b">
                            <img
                            class="h-10 w-10 rounded-full object-cover object-center mr-3"
                            src="https://randomuser.me/api/portraits/men/{{$randomNumber}}.jpg"
                            alt=""
                            />
                            {{$user->nom}}
                        </td>
                        <td class="px-4 py-3 border-b">{{$user->prenom}}</td>
                        <td class="px-4 py-3 border-b">{{$user->email}}</td>
                        <td class="px-4 py-3 border-b">{{$user->numTelephone}}</td>
                        <td class="px-4 py-3 border-b">
                            <div class="flex justify-center rounded items-center space-x-2 text-sm shadow-xl p-0.5 bg-gray-100">
                                <a href="{{ route('admin.users.show', $user->id) }}" class="text-maquette-gris">
                                ...
                                <a href="{{route("admin.users.edit", $user->id)}}" class="text-first-orange">
                                ...
                                <form action="{{route("admin.users.destroy", $user->id)}}" method="POST" class="inline">
                                    @csrf
                                    @method("DELETE")
                                    <button type="submit" class="text-first-orange">
