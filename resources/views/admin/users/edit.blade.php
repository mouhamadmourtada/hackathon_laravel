<x-app-layout>
    ...
    <div class="flex mb-4 text-sm font-bold">
        ...
        <p class="text-secondaire">Edition d'un utilisateur</p>
    </div>
            ...
            <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                ...
                <div class="flex flex-wrap w-full justify-evenly">
                    ...
                    <div class="flex-grow mb-4 mr-2">
                        ...
                        <x-input id="nom" class="block w-full focus:border-secondaire enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="text" name="nom" value="{{ $user->nom }}" required autofocus autocomplete="nom" />
                        @error('nom')
                    ...
                    <div class="flex-grow mb-4 mr-2">
                        ...
                        <x-input id="prenom" class="block w-full focus:border-secondaire enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="text" name="prenom" value="{{ $user->prenom }}" required autofocus autocomplete="prenom" />
                        @error('prenom')
                            <span class="text-xs text-red-500">
                                {{$message}}
                            </span>
                    ...
                    <div class="flex-grow mb-4 mr-2">
                        ...
                        <x-input id="email" class="block w-full focus:border-secondaire enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="text" name="email" value="{{ $user->email }}" required autofocus autocomplete="email" />
                        @error('email')
                            <span class="text-xs text-red-500">
                                {{$message}}
                            </span>
                    ...
                    <div>
                        ...
                        <select id="role" name="role" class="block w-full focus:border-secondaire enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2">
                            <option value="">Select a role</option>
                            <option value="admin" {{ $user->role == 'agent' ? 'selected' : '' }}>Agent</option>
                            <option value="user" {{ $user->role == 'partenaire' ? 'selected' : '' }}>Partenaire</option>
                            <option value="user" {{ $user->role == 'particulier' ? 'selected' : '' }}>Particulier</option>
