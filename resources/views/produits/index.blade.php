                    ...
                    <div class="max-w-7xl mx-auto">
                        <div class="overflow-hidden shadow-xl sm:rounded-lg">
                            <div class="flex justify-between items-center">
                                <p class="font-bold py-2 m-4 text-secondaire text-xl">Produits</p>
                                ...
                                <div class="flex justify-end m-4 bg-secondaire hover:bg-white  text-white hover:text-secondaire hover:border hover:border-secondaire font-bold py-2 px-4 rounded items-center">
                                    <a href="{{ route('produits.create') }}" class="mr-2 text-sm">
                                        Nouveau produit
                                    </a>
                                    ...
                                    <table class="w-full border-t">
                                        <thead>
                                            <tr class="text-xs font-black tracking-wide text-left text-white uppercase border-b bg-secondaire">
                                                ...
                                                <th class="px-4 py-4">caégorie</th>
                                        ...
                                        <tbody class="bg-white divide-y">
                                            @foreach($produits as $produit)
                                                <tr class="text-gray-700 ">
                                                    <td class="px-4 py-3 border-b">{{ $produit->nom }}</td>
                                                    <td class="px-4 py-3 border-b">{{ $produit->libelle }}</td>
                                                    <td class="px-4 py-3 border-b">{{ $produit->prixBase }}</td>
                                                    {{-- <td class="px-4 py-3 border-b">{{ $categorie->description }}</td> --}}
                                                    <td class="px-4 py-3 border-b">{{ $produit->categorie->libelle }}</td>
    
                                                    <td class="px-4 py-3 border-b">
                                                        <div class="flex justify-center rounded items-center space-x-2 text-sm shadow-xl p-0.5 bg-gray-100">
                                                            <a href="{{ route('produits.show', $produit->id) }}" class="text-maquette-gris">
                                                            ...
                                                            <a href="{{ route('produits.edit', $produit->id) }}" class="text-purple-600">
                                                            ...
                                                            <form class="text-center" action="{{ route('produits.destroy', $produit->id) }}" method="POST">
