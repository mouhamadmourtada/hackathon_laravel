<x-app-layout>
    ...
    <div class="flex mb-4 text-sm font-bold">
        ...
        <p class="text-first-orange">Editer catégorie</p>
    </div>
            ...
            <form action="{{ route('categories.update', $categorie->id) }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                ...
                <div class="flex flex-wrap w-full justify-evenly">
                    ...
                    <div class="flex-grow mb-4 mr-2">
                        ...
                        <x-input id="libelle" class="block w-full focus:secondaire enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="text" name="libelle" value="{{ $categorie->libelle }}" required autofocus autocomplete="libelle" />
                        @error('libelle')
                ...
                <div class="mb-4 mx-auto w-full">
                    ...
                    <textarea value="{{ $categorie->description }}" class="enlever_shadow shadow appearance-none border-2 border-gray-400 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:border-orange-500" id="description" name="description" required rows="4" placeholder="Description">{{$categorie->description}}</textarea>
                    @error('description')
                        <span class="text-xs text-red-500">
                            {{$message}}
                        </span>
                ...
                <button type="submit" class="flex items-center bg-secondaire text-white rounded-md px-5 py-2 hover:bg-white hover:text-secondaire hover:border-secondaire hover:border">
